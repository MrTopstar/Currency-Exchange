...
class product_list extends CI_Controller{
    ...
    public function product_list(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['settingsvalue'] = $this->product_list_model->getSettingsValue();
            $data['country']         = $this->product_list_model->getCountry();
            $data['category']      = $this->product_list_model->getCategory();
            $data['sub_category']      = $this->product_list_model->getSubcategory();
            $data['color']         = $this->product_list_model->getColor();
            $data['size']          = $this->product_list_model->getSize();
            $data['brand']         = $this->product_list_model->getBrand();
            $data['proimage']      = $this->product_list_model->getProImage();
            $data['productlist']   = $this->product_list_model->getProductData();   
    ...
    public function Delet_ProductInfo(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $value = $this->product_list_model->getProductById($id);
            if (!empty($value)) {
                $deletproduct = $this->product_list_model->delet_Product($id);
                $deletcolor   = $this->product_list_model->delet_Color($id);
                $deletsize    = $this->product_list_model->delet_Size($id);
                $imgvalue     = $this->product_list_model->getProImageById($id);
                if (!empty($imgvalue)) {
                    ...
                    $delet = $this->product_list_model->deelet_Pro_Imgage($id);
                }
    ...
    public function product_details(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data                   = array();
            $data['settingsvalue']  = $this->product_list_model->getSettingsValue();
            $data['productdetails'] = $this->product_list_model->getproductdetails($proid);
            $data['productsize']    = $this->product_list_model->getproductsize($proid);
            $data['productcolor']   = $this->product_list_model->getproductcolor($proid);
            $data['productimage']   = $this->product_list_model->getproductImage($proid);
    ...
    public function Getprodatabyid(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['brand']         = $this->product_list_model->getBrand();
            $data['country']         = $this->product_list_model->getCountry();
            $data['productvalue']  = $this->product_list_model->getproductdetails($proid);
            $data['productimage']  = $this->product_list_model->getproductImages($proid);
            $data['productcolor']  = $this->product_list_model->getProductColors($proid);
            $data['productsize']   = $this->product_list_model->getProductSizes($proid);
    ...
    public function addProductData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $proid       = 'P' . rand(0, 1000);
            $country_id  = $this->input->post('cotid');
            ...
            $this->form_validation->set_rules('cotid', 'Country', 'trim|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 0; $i < $cpt; $i++) {
                    ...
                    if ($this->upload->do_upload('product_image')) {
                        ...
                        $success                     = $this->product_list_model->productImgInsert($data1);
                    }
                }
                if (!empty($uploadData)) {
                    $data                = array();
                    ...
                    $success             = $this->product_list_model->productInsert($data);
                    ...
                    if ($insertid) {
                        ...
                        foreach ($color as $colorvalue) {
                            ...
                            $success     = $this->product_list_model->productColor($data);
                        ...
                        foreach ($size as $sizevalue) {
                            ...
                            $success       = $this->product_list_model->productSize($data);
        ...
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    ...
                    $success    = $this->product_list_model->productUpdateInfo($id, $data);
                    $deletcolor = $this->product_list_model->delet_Color($id);
                    $deletsize  = $this->product_list_model->delet_Size($id);
                    $color      = $this->input->post('color[]');
                    $size       = $this->input->post('size[]');
    ...
    public function getSubcategoryByid(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['subcat'] = $this->product_list_model->getSubCatById($id);
    ...
    public function addSubCategoryData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id)) {
                    $update              = $this->product_list_model->updateSubcategory($id, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->product_list_model->insertSubcategory($data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Added";
                    $this->output->set_output(json_encode($response));
                }
    ...
    public function getCategoryByID(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $subcatlist = $this->product_list_model->getsubcategoryByID($catid);
