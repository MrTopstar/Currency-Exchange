...
class currencyCode extends CI_Controller
{
    
    function __construct(){
        parent::__construct();
        $this->load->database();
        $this->load->model('currencyCode_model');
     ...
     public function view_currency()
    {
        ...
        {
            $data             = array();
            $data['category'] = $this->currencyCode_model->getCurrency();
    ...
    public function currency_delet(){
        if ($this->session->userdata('user_login_access') != False) {
            $id = $this->input->get('id');
            $this->currencyCode_model->currencyTableDelet($id);
            if ($this->db->affected_rows()) {
                $profile    = $this->currencyCode_model->getCurrencyValue($id);
                $checkimage = "./assets/img/user/$profile->image";
                if (file_exists($checkimage)) {
                    unlink($checkimage);
                    redirect('currencyCode/currency_delet');
                }
    ...
    public function Delet_CurrencyInfo(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $value = $this->currencyCode_model->getCurrencyById($id);
            if (!empty($value)) {
                $deletproduct = $this->currencyCode_model->delet_currency($id);
                
                
                redirect('currencyCode/view_currency');
            } else {
