...
class cuex extends CI_Controller
{
    
    function __construct(){
        parent::__construct();
        $this->load->database();
        $this->load->model('cuex_model');
    ...
    public function addCurrencyData(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($catid)) {
                    $update              = $this->cuex_model->updateCurrency($catid, $data);
                    $response['status']  = 'success';
                    $response['message'] = "Successfully Updated";
                    $this->output->set_output(json_encode($response));
                } else {
                    $insert              = $this->cuex_model->insertcurrency($data);
                    $response['status']  = 'success';
    ...
    public function getCurrencyById(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['brandvalue'] = $this->cuex_model->getCurrencyBYID($id);
        ...
        if ($this->session->userdata('user_login_access') != False) {
            $id                 = $this->input->get('id');
            $data['currency'] = $this->cuex_model->getCurrencyrateBYID($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
     ...
     public function view_currency()
    {
        ...
        {
            $data             = array();
            $data['category'] = $this->cuex_model->getCurrencyRate();
            $data['currency'] = $this->cuex_model->getCurrency();
    ...
    public function currencyById()
    {
        ...
        {
            $id               = $this->input->get('id');
            $data['catvalue'] = $this->cuex_model->getCurrencyValueById($id);
            echo json_encode($data);
        } 
        else 
        {
    ...
    function set_notification(){
        $data = $_POST["id"];
        $this->load->model('cuex_model');
        $this->cuex_model->set_notifiication($data);
    }
    public function currency_delet(){
        if ($this->session->userdata('user_login_access') != False) {
            $id = $this->input->get('id');
            $this->cuex_model->currencyTableDelet($id);
            if ($this->db->affected_rows()) {
                $profile    = $this->cuex_model->getCurrencyValue($id);
                $checkimage = "./assets/img/user/$profile->image";
                if (file_exists($checkimage)) {
                    unlink($checkimage);
                    redirect('cuex/currency_delet');
                }
    ...
    public function Delet_CurrencyInfo(){
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $value = $this->cuex_model->getCurrencyById($id);
            if (!empty($value)) {
                $deletproduct = $this->cuex_model->delet_currency($id);
                
                
                redirect('cuex/view_currency');
            } else {
